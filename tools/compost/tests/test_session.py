...
from compost.session import format_session, find_session
    ...
    _write_jsonl(p, [
        ...
        },
        {
            ...
            "message": {"content": [
                ...
                {"type": "tool_use", "id": "id1", "name": "mcp__compost__load_service_context",
...
def test_format_session_full(tmp_path):
    ...
    assert "mcp__compost__load_service_context" in output
...
def test_find_session(tmp_path, monkeypatch):
    monkeypatch.setattr("compost.session.Path.home", lambda: tmp_path)
